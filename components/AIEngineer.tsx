...
import { Bot, Send, Terminal, User, Loader2, Sparkles, Zap, Lightbulb, Plus, Search, FileText, MessageSquare, Rocket, DollarSign, Layers, Target, MoveRight, CheckCircle2, ArrowRight, Maximize2, Download, Share2, Code, Hammer } from 'lucide-react';
import { ChatMessage, AppIdea } from '../types';
import { askAIEngineer } from '../services/huggingFaceService';
import ReactMarkdown from 'react-markdown';
import { fetchAppIdeas, createAppIdea, createProject } from '../services/apiClient';
...
const AppIdeas: React.FC = () => {
   ...
   const handleDownload = () => {
      ...
      const data = JSON.stringify(selectedIdea, null, 2);
      const blob = new Blob([data], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${selectedIdea.title.toLowerCase().replace(/\s+/g, '-')}-blueprint.json`;
      a.click();
   ...
   const handleProvision = async () => {
      ...
      try {
         const proj = await createProject({
            ...
            repoUrl: `https://github.com/vibe-coder/${selectedIdea.title.toLowerCase().replace(/\s+/g, '-')}`,
            techStack: selectedIdea.tags,
         });
         alert(`Project "${proj.name}" provisioned in Dev Hub.`);
      } catch (err) {
         console.error('Provision failed', err);
         alert('Failed to provision: ' + (err instanceof Error ? err.message : 'Unknown error'));
      } finally {
         setCreating(false);
      }
   };
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
               ...
               {ideas.map(idea => (
                  <div
                     ...
                     className={`p-4 rounded-xl cursor-pointer transition-all group border relative overflow-hidden ${selectedIdeaId === idea.id
            ...
            <div className="h-16 border-b border-white/5 flex items-center justify-between px-6 bg-zinc-950/50 backdrop-blur-md sticky top-0 z-30">
               ...
               <div className="flex bg-black/50 p-1 rounded-lg border border-white/10">
                  <button
                     ...
                     className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all flex items-center ${activeTab === 'canvas'
                  ...
                  <button
                     ...
                     className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all flex items-center ${activeTab === 'chat'
                              ...
                              <div className="flex items-center justify-between mb-6">
                                 ...
                                 <div className="flex items-center space-x-2">
                                    ...
                                    <button
                                       ...
                                       disabled={creating}
                                       className="flex items-center px-4 py-2 bg-white text-black rounded-xl text-xs font-bold hover:bg-indigo-50 transition-all shadow-lg select-none disabled:opacity-50"
                                    >
                                       {creating ? <Loader2 className="w-3.5 h-3.5 animate-spin mr-2" /> : <Hammer className="w-3.5 h-3.5 mr-2" />}
                                       Review & Provision
                                    </button>
                                    <button
                           ...
                           <div className="bg-[#0c0c0c] border border-white/5 rounded-3xl overflow-hidden relative group">
                              <div className="p-6 border-b border-white/5 flex items-center justify-between bg-zinc-900/20">
                                 <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-widest flex items-center">
                                    <MoveRight className="w-4 h-4 mr-2 text-indigo-500" /> System Topology
                                 </h3>
                                 <div className="flex items-center gap-2">
                                    <button
                                       onClick={() => setShowMermaidCode(!showMermaidCode)}
                                       className="px-2 py-1 hover:bg-white/10 rounded text-[10px] font-bold text-zinc-500 hover:text-zinc-300 transition-colors uppercase tracking-wider border border-white/5"
                                    >
                                       {showMermaidCode ? 'View Diagram' : 'View Core Content'}
                                    </button>
                                 </div>
                              </div>

                              <div className="p-8 bg-black/50 flex justify-center min-h-[400px]">
                                 {showMermaidCode ? (
                                    <pre className="text-xs font-mono text-zinc-500 w-full overflow-auto bg-black/40 p-4 rounded-xl border border-white/5 whitespace-pre-wrap">
                                       {selectedIdea.mermaidDiagram}
                                    </pre>
                                 ) : (
                                    <div className="w-full">
                                       <Mermaid chart={selectedIdea.mermaidDiagram} />
                                    </div>
                                 )}
                              </div>
                           </div>
                        ...
                        {messages.map((msg) => (
                           <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-4 duration-300`}>
                              <div className={`flex max-w-[85%] lg:max-w-[70%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                                 ...
                                 {/* Avatar */}
                                 <div className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center mt-1 shadow-lg ${msg.role === 'user'
                                    ? 'bg-white ml-3'
                                 ...
                                 {/* Bubble */}
                                 <div className={`rounded-2xl text-sm leading-relaxed shadow-md border relative group ${msg.role === 'user'
